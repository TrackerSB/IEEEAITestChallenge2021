...
The execution of the generated test cases resulted in failed test cases caused by Apollo not reaching the target location in time. Since we take extra care to remove all sources for false positives (e.g., traffic lights, random traffic, etc.), this result suggests that Apollo cannot plan a suitable trajectory.
...
We tested SALVO on **Ubuntu 20.04.2 LTS** running the **SVL Simulator version 2021.01** and **Apollo 6.0**. We used the SVL Python interface, the Lincoln2017MKZ vehicle, and the following maps from SVL:
...
The first command generates **all** the paths, i.e., legal trajectories, across the intersections that SALVO finds in the given `map-file`, without filtering and without executing the test cases. Each path starts before the intersection, crosses the intersection (going straight, turning left or right), and ends after it.
The parameters `before-junction` and `after-junction` controls the distance at which the ego-car is initially placed and the location of the target position after the intersection.
...
The sample command configures SALVO to use the Cube Town map and start/end the paths 20 meters before/after the intersection. Since there are two intersections, each having three incoming roads and two possible directions, inside Cube Town this command should generate 12 paths (see [`trips-routes/cubetown-maps.jpg`](trips-routes/cubetown-maps.jpg)).
...
The second command instead uses the feature maps [2] to characterize the generated paths and filters them based on the cells they cover: if two paths cover the same cells in the feature map, on of them is discarded. The sample command configures SALVO to use 10-by-10 maps and should produce five paths (see [`trips-routes/cubetown-maps-feature-10.jpg`](trips-routes/cubetown-maps-feature-10.jpg)).
...
Finally, we can configure SALVO to generate test cases with all the features enabled:
...
- [Static Obstacles in CubeTown](https://youtu.be/WLIh9Pv8FcI). This video shows four tests created by SALVO in the CubeTown map. The tests cover various combinations of static obstacle placement: no obstacle, an obstacle in the middle of the lane, and obstacles partially occluding the lane on the left and right sides.
