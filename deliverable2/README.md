...
The python modules to generate, filter, and execute the test cases are available in this repo. The main script is `salvo.py` while the supporting modules can be found under `models`.

We do not provide the description of any *predefined* scenarios, trips or routes, as SALVO generates all of them automatically at runtime. Nevertheless, we report examples of the generated routes inside the folder `trips-routes`. 
These visualization show the starting and ending position of the ego-car in the context of the map .

- [ ] **TODO: Plot the intersection and the trip.route is possible!!**

Likewise, we report the simulation execution reports for some of the generated  scenario executions inside the folder `simulation reports`. 
Links to the videos illustrating SALVO in actions are listed below.

- [ ] **TODO: Add more videos and descriptions!!**

> NOTE: Those examples include scenarios that take place at various intersections inside maps provided with the SVL simulators; some of those scenarios include static obstacles (e.g., parked NPC).
...
We start from the consideration that manually generated HD maps are very expensive; therefore, testers must use (and re-use) them wisely and extensively. Consequently, we designed SALVO to leverage existing maps and generate as many tests as possible. However, generating many tests without guidance is not cost-effective as not all the tests may be relevant and many of them may be too similar. Therefore, SALVO follows a systematic approach to generate relevant test cases and maximize their diversity. In this work, we consider driving scenarios relevant only if the require the ego-car to drive through an intersection.
...
yes, I made them always green to prevent the car stopping unexpectedly and then it might be out of timeout which leads to Test failed
...
- [x] **TODO CHECK THAT ALL THE PACKAGES AND VERSIONS ARE LISTED HERE**
...
- [x] **TODO Add the versions of the OS**
...
- [x] **TODO: Check if parameters are map_file or map-file, usually the format uses the hyphen (-) not the underscore (_)**
...
- [x] **TODO Check that 5 meters are from the ego car or the intersection... no idea... ** => 5m from the intersection
