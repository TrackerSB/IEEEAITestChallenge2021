...
def _generate_initial_ego_state() -> Tuple[AgentState, float]:
    ...
    time_to_crash = pedestrian_crash_distance / (PEDESTRIAN_SPEED / 3.6)  # in seconds
    ego_crash_distance = (EGO_SPEED / 3.6) * time_to_crash
    ego_spawn = Spawn(Transform(position=EGO_CRASH_POS + Vector(0, 0, ego_crash_distance), rotation=EGO_ORIENTATION))
    return generate_initial_state(ego_spawn, EGO_SPEED), time_to_crash
...
def _main() -> None:
    ...
    initial_ego_state, time_to_crash = _generate_initial_ego_state()
